...
def image_path_list(path1, seed_num=0):
    ...
    # 遍历三次得到该路径下的所有图片路径
    for j in classificasion_list:
        image_list = os.listdir(path1 + '/' + j)
        image_path = path1 + '/' + j
        for k in image_list:
...
def get_dict(path):
    ...
    for i in object_list:
        object_dict[i] = object_list.index(i)
    return object_dict
...
def image_to_target(target_num_path, seed_num=0):
    target_list = []
    target_dic = get_dict(target_num_path)
    classificasion_list = os.listdir(target_num_path)
    # 遍历2次得到该路径下的所有图片路径并且根据文件夹的不同对不同的图片打上对应的标签
    for j in classificasion_list:
        target = target_dic[j]
        image_list = os.listdir(target_num_path + '/' + j)
        for k in image_list:
