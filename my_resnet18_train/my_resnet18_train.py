...
# 可改变参数字典
parameter_18 = {
    'data_path': 'C:/Users/sunshenao/deeplearn/data/rubbish_catalog/kehuishoulaji',  # 数据路径
    'tensorboard_save_path': '/tmp/pycharm_project_692/my_resnet18_train/logs_1/obtain_best_model',    # tensorboard图保存路径
    ...
    'use_cos_or_not': True,  # 是否对学习率使用余弦退火算法改动
    ...
    'model_save_path': './model1.pth',  # 模型参数文件保存路径
...
if __name__ == '__main__':
    ...
    path = parameter_18['data_path']
    # path = '/data/qiucx/rubbish_catalog'
    write = SummaryWriter(parameter_18['tensorboard_save_path'])
    # 如果选择使用cuda训练先测试能够使用gpu，能就使用，不能则返回错误并且改用cpu
    if parameter_18['train_device'] == 'cuda':
        if torch.cuda.is_available():
            device = torch.device('cuda')
        else:
            device = torch.device('cpu')
            print('无法使用gpu，改用cpu')
    elif parameter_18['train_device'] == 'cpu':
        print('使用cpu')
        device = torch.device('cpu')
    else:
        device = torch.device('cpu')
    ...
    dict1 = get_dict(parameter_18['data_path'])
    out_num = len(dict1.keys())
    print(out_num)
    # 模型加载,断点续训
    if os.path.exists(parameter_18['model_save_path']):
        print('------------------------------------------加载模型参数--------------------------------------------')
        model = My_resnet_18(out_num=out_num)
        model.load_state_dict(torch.load(parameter_18['model_save_path']))
        print('------------------------------------------加载模型完毕--------------------------------------------')
    else:
        print('-------------------------------------------创建新模型---------------------------------------------')
        model = My_resnet_18(out_num=out_num)
    ...
    # 选择损失函数
    if parameter_18['loss_function'] == 'CrossEntropy':
        loss_fn = torch.nn.CrossEntropyLoss()
        print('loss选择CrossEntropy')
    elif parameter_18['loss_function'] == 'L1Loss':
        loss_fn = torch.nn.L1Loss()
        print('loss选择L1loss')
    elif parameter_18['loss_function'] == 'MSELoss':
    ...
    # 设置学习率
    learn_rate = parameter_18['learn_rate']
    print("初始学习率为:", learn_rate)
    if parameter_18['optim'] == 'Momentum':
    ...
    # 学习率采用退火余弦算法改变
    if parameter_18['use_cos_or_not']:
        print('余弦退火')
        torch.optim.lr_scheduler.CosineAnnealingLR(optim, parameter_18['lr_cos_period'], eta_min=0, last_epoch=-1)
    elif not parameter_18['use_cos_or_not']:
        print('lr静态')
        pass
    else:
        print('你输入了违法字符')
    ...
    for i in range(1, epoch + 1):
        ...
        print(train_lenth)
        # 数据加载
        train_dataloader = DataLoader(train_data, batch_size=parameter_18['batch_size'],
                                      num_workers=parameter_18['num_worker'])
        test_dataloader = DataLoader(test_data, batch_size=parameter_18['batch_size'],
                                     num_workers=parameter_18['num_worker'])
        train_loss_all = 0
        batch_loss_all = 0
        star_time = time.time()
        ...
        for data in train_dataloader:
            ...
            if parameter_18['loss_function'] == 'L1Loss' or parameter_18['loss_function'] == 'MSELoss':
                result_loss = loss_fn(output, F.one_hot(targets, num_classes=4).float())
            elif parameter_18['loss_function'] == 'CrossEntropy':
    ...
    print(dict1)
    for i in dict1.keys():
        file = open('./dict1.txt', 'a', encoding='utf_8')
        file.write(i + ':' + str(dict1[i]) + '\n')
    file.write(str(out_num))
    file.close()
